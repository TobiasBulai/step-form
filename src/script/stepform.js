...
const DrawEventHandler = class {
  ...
  drawInit (cube) {
    let x = 0
    const spacing = this.#calculateSpacing(cube)
  ...
  #calculateSpacing (cube) {
    const totalWidthOfCubes = cube.width * (this.#numSteps)
    const spaceLeft = this.#canvas.width - totalWidthOfCubes
...
const StepForm = class {
  ...
  #buildStepPage (step, index) {
    ...
    const updateBasicStepCounter = function (stepIndex, numSteps) {
      const textElement = document.querySelector('div.status-field-basic p')
    ...
    if (index > 0) {
      ...
      const backEvent = function (event) {
        ...
        const nextStep = document.querySelectorAll('#stepform > form > div.step')
  ...
  #buildBasicStatusField () {
    const statusField = this.#createStatusFieldElement('status-field-basic')
